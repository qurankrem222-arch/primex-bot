...
FLAGS = {
    "india":"🇮🇳 India","egypt":"🇪🇬 Egypt","vietnam":"🇻🇳 Vietnam","ksa":"🇸🇦 KSA",
    "pakistan":"🇵🇰 Pakistan","morocco":"🇲🇦 Morocco","myanmar":"🇲🇲 Myanmar",
    "kenya":"🇰🇪 Kenya","indonesia":"🇮🇩 Indonesia","nigeria":"🇳🇬 Nigeria",
    "usa":"🇺🇸 USA","ghana":"🇬🇭 Ghana","sudan":"🇸🇩 Sudan",
    "germany":"🇩🇪 Germany","mozambique":"🇲🇿 Mozambique",
    "random":"🌍 Random","rare":"💎 Rare"
}
...
def start(msg):
    ...
    if not ok:
        ...
        txt="╔═══════ ⚠️ ═══════╗\n║ *Must Subscribe* ║\n╚════════════════╝\n\n"
        for ch in nj:
            m.add(types.InlineKeyboardButton(f"📢 {ch} - Join",url=f"https://t.me/{ch.replace('@','')}"))
        m.add(types.InlineKeyboardButton("📘 Facebook Page - Like 👍",url=FACEBOOK_PAGE))
        txt+="📢 Join Channels Above\n📘 Like Facebook Page\n\n✅ Then press Done"
        m.add(types.InlineKeyboardButton("✅ Done",callback_data="check"))
        bot.send_message(uid,txt,parse_mode="Markdown",reply_markup=m)
        return
    bal=round(get_balance(uid),2)
    bot.send_message(uid,WELCOME.format(bal=bal,uid=uid),parse_mode="Markdown",reply_markup=main_menu(uid))
...
def cb(call):
    ...
    if d=="check":
        ...
        if not ok:
            ...
            return
        bal=round(get_balance(uid),2)
        bot.edit_message_text(WELCOME.format(bal=bal,uid=uid),uid,call.message.message_id,parse_mode="Markdown",reply_markup=main_menu(uid))
        return

    if uid==ADMIN_ID:
        if d=="admin_stats":
            total=sum(float(v) for v in balance_cache.values())
            bot.edit_message_text(f"📊 *Stats*\n💰 Total: ${round(total,2)}\n👥 Users: {len(users_cache)}",uid,call.message.message_id,parse_mode="Markdown",reply_markup=admin_menu())
        elif d=="admin_add":
    ...
    if d=="back":
        ...
        return
    if d=="nums":
        bot.edit_message_text("╔══ 📱 *Numbers* 🌍 ══╗\n║ Choose category\n╚═══════════════╝",uid,call.message.message_id,parse_mode="Markdown",reply_markup=nums_menu())
        return
    if d=="clean": bot.edit_message_text("✨ *Clean SMS* ✨\n🌍 With Flags:",uid,call.message.message_id,parse_mode="Markdown",reply_markup=build_nums(CLEAN,"clean")); return
    if d=="spam": bot.edit_message_text("🔥 *Spam SMS* 🔥\n🌍 With Flags:",uid,call.message.message_id,parse_mode="Markdown",reply_markup=build_nums(SPAM,"spam")); return
    if d=="wa": bot.edit_message_text("💚 *WhatsApp* 💚",uid,call.message.message_id,parse_mode="Markdown",reply_markup=build_nums(WHATSAPP,"wa")); return
    if d=="tiktok": bot.edit_message_text("🎵 *TikTok* 🎵",uid,call.message.message_id,parse_mode="Markdown",reply_markup=build_nums(TIKTOK,"tiktok")); return
    if d=="fb": bot.edit_message_text("👤 *Facebook* 👤",uid,call.message.message_id,parse_mode="Markdown",reply_markup=build_nums(FACEBOOK,"fb")); return
    if d=="insta": bot.edit_message_text("📸 *Instagram* 📸",uid,call.message.message_id,parse_mode="Markdown",reply_markup=build_nums(INSTA,"insta")); return
    if d=="tinder": bot.edit_message_text("🔥 *Tinder* 🔥",uid,call.message.message_id,parse_mode="Markdown",reply_markup=build_nums(TINDER,"tinder")); return
    if d=="followers_main":
        bot.edit_message_text("╔══ 👥 *Followers* ══╗\n║ From $0.09\n╚══════════════╝",uid,call.message.message_id,parse_mode="Markdown",reply_markup=followers_platforms_menu())
        return
    if d.startswith("fp_"):
        platform=d.replace("fp_","")
        bot.edit_message_text(f"👥 *{platform.upper()}* - Choose:",uid,call.message.message_id,parse_mode="Markdown",reply_markup=followers_counts_menu(platform))
        return
    if d=="accounts_main":
        bot.edit_message_text("👤 *Accounts Store* 🔥",uid,call.message.message_id,parse_mode="Markdown",reply_markup=accounts_menu())
        return
    if d=="stars":
        m=types.InlineKeyboardMarkup(row_width=2)
        for k,price in STARS.items(): m.add(types.InlineKeyboardButton(f"⭐ {k} • ${price}",callback_data=f"buy_stars_{k}"))
        m.add(types.InlineKeyboardButton("⬅️ Back",callback_data="back"))
        bot.edit_message_text("⭐ *Telegram Stars* - Cheapest!",uid,call.message.message_id,parse_mode="Markdown",reply_markup=m)
        return
    if d=="recharge":
        m=types.InlineKeyboardMarkup(row_width=2)
        for k,(name,addr) in PAYMENTS.items(): m.add(types.InlineKeyboardButton(name,callback_data=f"pay_{k}"))
        m.add(types.InlineKeyboardButton("⬅️ Back",callback_data="back"))
        bot.edit_message_text(f"💳 *Recharge*\n💰 `${round(get_balance(uid),2)}`\n\nChoose:",uid,call.message.message_id,parse_mode="Markdown",reply_markup=m)
        return
    if d.startswith("pay_"):
        ...
        bot.edit_message_text(txt,uid,call.message.message_id,parse_mode="Markdown",reply_markup=types.InlineKeyboardMarkup().add(types.InlineKeyboardButton("⬅️ Back",callback_data="recharge")))
        return
    if d.startswith("buy_"):
        try:
            ...
            if parts[1]=="followers":
                ...
                cat=parts[1]
                country=parts[2]
                mp={"clean":CLEAN,"spam":SPAM,"wa":WHATSAPP,"tiktok":TIKTOK,"fb":FACEBOOK,"insta":INSTA,"tinder":TINDER}
                price=mp[cat][country]
                name=f"{cat} {country}"
            ...
            bot.send_message(ADMIN_ID,f"🔔 *New Order*\n👤 `{uid}`\n📦 {name}\n💰 ${price}",parse_mode="Markdown",reply_markup=m)
            bot.answer_callback_query(call.id,"✅ Sent to admin!")
            txt=f"╔══ ✅ *Order Sent* ══╗\n║ 📦 {name}\n║ 💰 ${price}\n║ ⏳ Waiting\n╚═══════════════╝"
            bot.edit_message_text(txt,uid,call.message.message_id,parse_mode="Markdown",reply_markup=main_menu(uid))
...
def all_msg(msg):
    ...
    if uid==ADMIN_ID and uid in admin_states:
        ...
        if state.startswith("await_amount_"):
            try:
                ...
                bot.send_message(uid,f"✅ Added ${amt} to {target}\nNew: ${round(new_bal,2)}",reply_markup=admin_menu())
                bot.send_message(target,f"╔══ ✅ *Recharged* ══╗\n║ 💰 ${amt}\n║ 💳 ${round(new_bal,2)}\n╚═══════════════╝",parse_mode="Markdown")
    ...
    if uid in pending_recharge:
        ...
        bot.send_message(ADMIN_ID,f"💳 *Recharge*\n👤 `{uid}`\n💳 {pending_recharge[uid]}",parse_mode="Markdown",reply_markup=m)
        bot.send_message(uid,"╔══ ✅ *Received* ══╗\n║ ⏳ Waiting admin\n╚═══════════════╝",parse_mode="Markdown")
        return
...
print("V45 FINAL PRETTY running @PrimeXStore00bot")
